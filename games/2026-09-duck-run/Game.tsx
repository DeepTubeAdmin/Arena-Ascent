...
export function drawFrame(
  ...
  if (airborne(st)) {
    ...
    const into = total - (st.jumpUntil - st.step);
    const t = into / total;                    // 0..1
    const lift = Math.floor(4 * 60 * t * (1 - t)); // parabola, peak 60px
    py = GROUND - ph - lift;
  } else if (st.ducking) {
    ph = 22; py = GROUND - ph;
  }
...
export default function DuckRunGame({ seed, onInput, onReady, started, onComplete }: GameProps) {
  ...
  useEffect(() => {
    ...
    function frame() {
      ...
      const targetStep = Math.min(MAX_STEPS, Math.floor((performance.now() - startRef.current) / STEP_MS));
